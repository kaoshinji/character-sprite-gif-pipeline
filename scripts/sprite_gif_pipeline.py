...
def rgb_distance(rgb: np.ndarray, key_rgb: Tuple[int, int, int]) -> np.ndarray:
    """Return Euclidean RGB distance without integer overflow.

    uint8/int16 RGB subtraction followed by squaring can overflow because
    255**2 is larger than int16's positive range.  Cast to int32 before the
    arithmetic so pure green and far-from-green pixels are classified
    correctly.
    """
    rgb32 = rgb.astype(np.int32, copy=False)
...
def chroma_key_to_alpha(
    ...
    dist = rgb_distance(arr[:, :, :3], key_rgb)
    mask = dist <= tolerance
